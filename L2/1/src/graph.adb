      ...
      begin
         ...
         if On_Vertex then
            ...
            for I in 0 .. Integer(Levels.Length - 1) loop
               ...
               if Level.used and then Level.from = Vertex_Id and then Level.to < Vertex_Id then
            ...
            for Next_Vertex_Id of g.verteces.Element(Vertex_Id).Edges loop
               if Next_Vertex_Id /= Vertex_Id + 1 and then Next_Vertex_Id > Vertex_Id then
                  ...
                  if not Found then
                     ...
                     Levels.Append((Next_Vertex_Id, Vertex_Id, True, False));
                  else
                     Level := Levels.Element(Next_Free_Spcace);
                     Level.used := True;
                     Level.to := Next_Vertex_Id;
                     Level.from := Vertex_Id;
            ...
            for From_Vertex in 0 .. IdType(Size(g) - 1) loop
               if (From_Vertex > Vertex_Id) then 
                  for To_Vertex of g.verteces.Element(From_Vertex).Edges loop
                     if (To_Vertex = Vertex_Id) then 
                        Next_Free_Spcace := 0;
                        Found := False;
                        ...
                        if not Found then
                           ...
                           Clear_Line := False;
                           Fill_To_Level := (if Fill_To_Level > Next_Free_Spcace then Fill_To_Level else Next_Free_Spcace);
                           Levels.Append((Vertex_Id, From_Vertex, True, False));
                        else
                           Level := Levels.Element(Next_Free_Spcace);
                           Level.used := True;
                           Level.to := Vertex_Id;
                           Level.from := From_Vertex;
                           Levels.Replace_Element(Next_Free_Spcace, Level);
