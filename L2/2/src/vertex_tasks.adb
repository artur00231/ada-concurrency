...
package body Vertex_Tasks is
   ...
   function simulate(g : in out graph.Graph; k : in Integer; h : in Integer) return Message_Set_Type.Set is
      ...
      task body Trap_Task is
         New_Trap_Delay : Duration := 10.0;

         subtype RandomRange is IdType range 0 .. IdType(Size(g));
         package Rand is new Ada.Numerics.Discrete_Random(RandomRange);
         use Rand;
         gen : Generator;
         Now : Ada.Calendar.Time := Ada.Calendar.Clock;
      ...
      begin
         ...
         Main_Loop: loop
            select
               ...
               exit Main_Loop;
            or
               delay New_Trap_Delay;
               Node_Id := Random(gen);

               Network_Locks(Node_Id).Seize;
               Network_Map(Node_Id) := -2;
               Network_Locks(Node_Id).Release;
            end select;
         end loop Main_Loop;
      end Trap_Task;
